...
-- 20181131 修改dba_hist_sqlstat显示的内容
...
COL AVG_IO_SPEED            heading "AVG(MS)|IO_SPEED"   FOR 999,999
...
SELECT TO_CHAR (END_INTERVAL_TIME, 'dd hh24') end_time,
       TRIM (a.instance_number) instance_number,
                                ...
                                a.plan_hash_value,
                                CASE
                                    WHEN a.executions_delta < 1000 THEN SUBSTR (a.executions_delta, 1, 4)
                                    WHEN a.executions_delta BETWEEN 1000 AND 10000 THEN SUBSTR (a.executions_delta / 1000, 1, 3) || 'K'
                                    WHEN a.executions_delta > 10000 THEN SUBSTR (a.executions_delta / 10000, 1, 3) || 'W'
                                END executions,
                                TRUNC ((buffer_gets_delta) / DECODE ((executions_delta), 0, 1, (executions_delta))) GET_PRE_EXEC,
                                TRUNC ((disk_reads_delta) / DECODE ((executions_delta), 0, 1, (executions_delta))) DISK_PRE_EXEC,
                                TRUNC ((direct_writes_delta) / DECODE ((executions_delta), 0, 1, (executions_delta))) WRITE_PRE_EXEC,
                                CASE
                                    WHEN rows_processed_delta < 1000 THEN SUBSTR (rows_processed_delta, 1, 4)
                                    WHEN rows_processed_delta BETWEEN 1000 AND 10000 THEN SUBSTR (rows_processed_delta / 1000, 1, 3) || 'K'
                                    WHEN rows_processed_delta > 10000 THEN SUBSTR (rows_processed_delta / 10000, 1, 3) || 'W'
                                END row_processes,
                                TRUNC ((rows_processed_delta) / DECODE ((executions_delta), 0, 1, (executions_delta))) ROWS_PRE_EXEC,
                                TRUNC ((fetches_delta) / DECODE ((executions_delta), 0, 1, (executions_delta))) ROWS_PRE_FETCHES,
                                TRUNC (((IOWAIT_DELTA) / DECODE ((executions_delta), 0, 1, (executions_delta)))/1000) USER_ID_WAIT_PER,
                  &_VERSION_11  TRUNC (((IOWAIT_DELTA) / DECODE ((physical_read_requests_delta+physical_write_requests_delta), 0, 1, (physical_read_requests_delta+physical_write_requests_delta)))/1000) AVG_IO_SPEED,
                                TRUNC (((elapsed_time_delta) / DECODE ((executions_delta), 0, 1, (executions_delta)))/1000) ELA_PRE_EXEC,
                                TRUNC (((cpu_time_delta) / DECODE ((executions_delta), 0, 1, (executions_delta)))/1000) CPU_PRE_EXEC,
                                TRUNC (((CLWAIT_DELTA) / DECODE ((executions_delta), 0, 1, (executions_delta)))/1000) CLU_WAIT_PER,
                                TRUNC (((PLSEXEC_TIME_DELTA) / DECODE ((executions_delta), 0, 1, (executions_delta)))/1000) PLSQL_WAIT_PER
...
tt AS
 (SELECT /*+materialize  no_merge */
  DISTINCT table_owner, table_name
    FROM dba_indexes
   WHERE (owner, index_name) IN (SELECT object_owner, object_name FROM t)),
ttt as
  (select /*+materialize  no_merge */ distinct object_owner,object_name from (
    select object_owner, object_name from t
    union
    select table_owner,table_name from dba_indexes where (owner, index_name) IN (SELECT object_owner, object_name FROM t)))
SELECT owner,
       segment_name,
       segment_type,
       TRUNC(SUM(bytes / 1024 / 1024)) s_size
  from (select owner,
               (SELECT '***'
                 ...
                 WHERE a.owner = tt.table_owner
                   AND a.segment_name = tt.table_name) || segment_name segment_name,
               segment_type,
               bytes
          FROM (SELECT owner,
                       segment_name,
         ...
         WHERE (a.owner, a.segment_name) IN
               (SELECT object_owner, object_name FROM ttt))
 GROUP BY owner, segment_type, segment_name
